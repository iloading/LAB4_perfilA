<section class="bg-light" id="member_details">
    <div class="container">
        <div class="row">
            <div class="col-lg-12 col-sm-6 text-center"> <img height="320" width="320" class="img-circle img-responsive img-center" src="images/team/<?= $mentor_image; ?>" alt="<?= $mentor_name; ?>">
                <h2 class="section-heading text-uppercase"><?= $mentor_name; ?></h2>
                <h3 class="section-subheading text-muted mb-2"><?= $mentor_title; ?></h3>
                <p><?= $mentor_description; ?></p>
            </div>
        </div>
    </div>
</section>